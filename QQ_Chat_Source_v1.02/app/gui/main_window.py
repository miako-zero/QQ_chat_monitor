...
import sys
import time as _time_module_for_media
import re
import secrets
from pathlib import Path
from urllib.parse import quote, urlparse

from PyQt6.QtCore import QPoint, QSize, QThread, QTimer, Qt, QUrl, pyqtSignal
...
from app.core.archive import MediaItem, list_media_dates, scan_media
...
class MediaScanWorker(QThread):
    ...
    def __init__(self, date_filter: str | None, media_filter: str, parent=None) -> None:
    ...
    def run(self) -> None:
        try:
            dates = list_media_dates()
            items = scan_media(date_filter=self.date_filter)
            if self.media_filter == "图片":
                items = [item for item in items if item.media_type == "image"]
            elif self.media_filter == "视频":
                items = [item for item in items if item.media_type == "video"]
            self.finished.emit(items, dates, self.date_filter or "")
...
class MainWindow(QMainWindow):
    def __init__(self) -> None:
        ...
        # 移除启动后全量扫描，改为用户主动切换页面时加载
        self._log_initialized = False

        self.media_timer: QTimer | None = None
        self.media_icon_timer: QTimer | None = None
        self.media_scan_worker: MediaScanWorker | None = None
        self.media_loaded_once = False

        # 日志定时器：只在用户切换到日志页面后启动
        self.log_timer = QTimer(self)
        self.log_timer.setInterval(2000)
        self.log_timer.timeout.connect(self._lazy_refresh_logs)
...
<h3>按钮说明</h3>
    ...
    def _build_archive_page(self) -> QWidget:
        # 聊天记录归档：切换到该页时触发首次加载
        self.chat_archive_loaded = False
        try:
            viewer = ArchiveViewer(self)
            self.archive_viewer = viewer
            return viewer
    ...
    def _switch_page(self, index: int) -> None:
        ...
        # 聊天记录归档页面（index=6）：首次加载延时触发
        if index == 6 and hasattr(self, "archive_viewer") and not self.chat_archive_loaded:
            self.chat_archive_loaded = True
            from PyQt6.QtCore import QTimer as _Q
            _Q.singleShot(50, self.archive_viewer.load_if_needed)
        # 控制日志定时器：只在日志页面可见时刷新
        self._log_page_visible = (index == 5)
        # 控制缩略图定时器：切到媒体页（index=1）时启动，切走时停止并释放缓存
        if index == 1:
            if not hasattr(self, "media_icon_timer") or self.media_icon_timer is None:
            ...
            if not self.media_icon_timer.isActive():
                self.media_icon_timer.start()
                self._schedule_media_icon_loading()
            if not hasattr(self, "media_timer") or self.media_timer is None:
                self.media_timer = QTimer(self)
                self.media_timer.setInterval(5000)
                self.media_timer.timeout.connect(self.refresh_media)
            if not self.media_timer.isActive():
                self.media_timer.start()
                self.refresh_media()
        else:
            if hasattr(self, "media_icon_timer") and self.media_icon_timer is not None and self.media_icon_timer.isActive():
                self.media_icon_timer.stop()
            if hasattr(self, "media_timer") and self.media_timer is not None and self.media_timer.isActive():
                self.media_timer.stop()
            # 切走时释放缩略图缓存和视频资源
            self.media_icon_cache.clear()
            self.media_icon_queue.clear()
            self.video_thumb_queue.clear()
    ...
    def refresh_media(self, *args) -> None:
        if not hasattr(self, "media_list"):
            return
        if self.media_scan_worker is not None and self.media_scan_worker.isRunning():
            return
        selected_date = self.media_date_filter.currentText() if hasattr(self, "media_date_filter") and self.media_date_filter.count() else ""
        if not selected_date:
            dates = list_media_dates()
            selected_date = dates[0] if dates else ""
        date_filter = None if selected_date == "全部日期" else selected_date
        filter_text = self.media_filter.currentText() if hasattr(self, "media_filter") else "全部"
        # 不清空列表，只设置状态提示；后台加载完成后填充
        self.media_detail.setPlainText(f"正在加载 {selected_date or '全部日期'} 的媒体列表...")
        self.media_scan_worker = MediaScanWorker(date_filter, filter_text, self)
        self.media_scan_worker.finished.connect(self._on_media_scan_finished)
        self.media_scan_worker.failed.connect(self._on_media_scan_failed)
        self.media_scan_worker.start()

    def _on_media_scan_failed(self, message: str) -> None:
        self.media_list.clear()
        self.media_detail.setPlainText(f"加载媒体失败: {message}")
        self.media_scan_worker = None

    def _on_media_scan_finished(self, items: list[MediaItem], dates: list[str], selected_date: str) -> None:
        self.media_scan_worker = None
        if hasattr(self, "media_date_filter"):
            current_date = self.media_date_filter.currentText() if self.media_date_filter.count() else selected_date
            if not current_date:
                current_date = selected_date or (dates[0] if dates else "")
            ...
            if current_date and self.media_date_filter.findText(current_date) >= 0:
                self.media_date_filter.setCurrentText(current_date)
            elif dates:
                self.media_date_filter.setCurrentText(dates[0])
            self.media_date_filter.blockSignals(False)
        self._populate_media_items(items)

    def _populate_media_items(self, items: list[MediaItem]) -> None:
        ...
        # 保存滚动位置
        scrollbar = self.media_list.verticalScrollBar()
        old_scroll = scrollbar.value() if scrollbar else 0
        self.media_list.clear()
        self.media_icon_queue = []
        self.video_thumb_queue = []
        self.video_thumb_current_row = None
        self.media_detail.setPlainText(f"已加载 {len(items)} 个媒体文件。选择一个媒体文件进行预览。")
        if hasattr(self, "video_thumb_player"):
            self.video_thumb_player.stop()
        ...
        if scrollbar and old_scroll > 0:
            QTimer.singleShot(10, lambda: scrollbar.setValue(old_scroll))
        self._schedule_media_icon_loading()
    ...
    def _load_more_media_icons(self) -> None:
        ...
        while self.media_icon_queue and loaded < batch_size:
            ...
            if icon is None:
                ...
                if len(self.media_icon_cache) > 300:
                    self.media_icon_cache.clear()
            if not icon.isNull():
                list_item.setIcon(icon)
            list_item.setData(MEDIA_ICON_READY_ROLE, True)
    ...
    def _lazy_refresh_logs(self, *args) -> None:
        """定时器调用的日志刷新：只在日志页面显示时执行，并限制显示行数。"""
        if not getattr(self, "_log_page_visible", False):
            return
        self._do_refresh_logs()

    def refresh_logs(self, *args) -> None:
        """用户手动刷新日志。"""
        self._log_page_visible = True
        self._do_refresh_logs()
    ...
    def _do_refresh_logs(self) -> None:
        ...
        if len(lines) > self._max_log_lines:
            ...
            text = "\n".join(lines)
        if text != current:
            self.file_log.setPlainText(text)
            if at_bottom:
